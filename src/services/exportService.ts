...
export async function vendorRegisterCsv(): Promise<string> {
  ...
  return toCsv(
    ...
    vendors.map((v: Vendor) => [String(v.id ?? ''), v.name, v.contact, v.email, v.dueDiligenceStatus, v.lastReviewDate, v.notes]),
  );
}
...
export async function generatePdfSummary(): Promise<Blob> {
  ...
  y += lh;
  doc.setFontSize(10);
  doc.text(`Open: ${incidents.filter((i) => i.status === 'open' || i.status === 'investigating').length}  |  Resolved: ${incidents.filter((i) => i.status === 'resolved' || i.status === 'closed').length}`, 14, y);
  y += lh + 4;
...
export async function generateIncidentPdf(incidentId: number): Promise<Blob> {
  ...
  doc.setFontSize(16);
  doc.text(`Incident Report — ${co}`, 14, y); y += 12;
